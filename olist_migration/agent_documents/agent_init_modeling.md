...
## Purpose
This document helps you resolve initialization and runtime issues when installing `kmds-modeling` into an Olist workspace such as `olist_migration`.

The package is designed to be installed into an existing KMDS workspace and used with the same CLI commands provided by this repository.

## Expected workflow
1. Install the package into the Olist workspace.
...
## Installation
From inside the Olist workspace environment:
...
```yaml
working_dir: /path/to/olist_migration
```
...
- Confirm `working_dir` is set and points to the Olist workspace root.
...
- Confirm the file exists:
  ```bash
  ls /path/to/olist_migration/data/featurization/model_ready_numeric_data.csv
  ```
